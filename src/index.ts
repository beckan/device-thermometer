...
const app = async () => {
  ...
  if (!doesConfigFileExist()) {
    printMessage.error(
      "Can't find .config.json file... Create it and try again."
    );
    return;
  }
  ...
  const temperatureSensor: Sensor = await getTemperatureSensor(
    Number(config.temperaturePollIntervall)
  );
